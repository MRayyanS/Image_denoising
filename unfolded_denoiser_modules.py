...
class MHA_Vision_Transformer_module(nn.Module):
    def __init__(self, ch_dimensions: list, num_att_heads: int):
        ...
        self.MH_Attention_module = Vision_Multi_Head_Attention(token_dim=ch_dimensions[0], att_emb_dim=ch_dimensions[1], att_out_dim=ch_dimensions[2], num_att_heads=num_att_heads)
    ...
    def forward(self, x):
        ...
        z = F.pixel_unshuffle(x, downscale_factor=2)
...
class MHA_Vision_Transformer_Block(nn.Module):
    def __init__(self, ch_dimensions: list, num_att_heads: int, num_modules: int):
        ...
        self.MHA_Vision_Transformer_modules = nn.ModuleList([
            MHA_Vision_Transformer_module(ch_dimensions, num_att_heads) for _ in range(num_modules)
        ])  
...
class MHA_Vision_Transformer_Denoiser(nn.Module):
    def __init__(self, patch_dim=7, token_dim=256, att_emb_dim=8, att_out_dim=64, mlp_middle_dim=16, num_att_heads=2, num_modules=3, im_color: str = 'color'):
        super().__init__()

        self.color_ch = 3 if im_color == 'color' else 1

        self.feature_extractor = Conv_Feature_Extraction(patch_dim=patch_dim, token_dim=token_dim, num_modules=1, color_ch=self.color_ch)
        self.Transformer_block = MHA_Vision_Transformer_Block(ch_dimensions=[token_dim, att_emb_dim, att_out_dim, mlp_middle_dim], num_att_heads=num_att_heads, num_modules=num_modules)
        self.reconstructor = Conv_Patch_Reconstruction(patch_dim=patch_dim, token_dim=token_dim, num_modules=1, color_ch=self.color_ch)
